...
export const cryptoAnalysisWorkflow = createWorkflowChain({
	...
	input: z.object({
		cryptoId: z
			.string()
			.describe("分析対象の暗号通貨ID（例：bitcoin, ethereum, cardano）"),
	...
	.andThen({
		...
		execute: async ({ data }) => {
			...
			const marketDataStatus = simulateMarketDataCollection(
				data.cryptoId as string,
			);
	...
	.andThen({
		...
		execute: async ({ data }) => {
			console.log(
				`📰 ${data.cryptoId as string}関連ニュースの収集と分析を開始...`,
			);

			// ニュース収集状況をシミュレート
			const newsAnalysisResult = simulateNewsCollection(
				data.cryptoId as string,
				data.newsCount as number,
			);

			console.log(
				`📊 ニュース分析結果: ${newsAnalysisResult.sentiment} (${newsAnalysisResult.articlesFound}件)`,
			);
	...
	.andThen({
		...
		execute: async ({ data }) => {
			...
			console.log(
				`📝 レポート生成完了 - リスク: ${riskAssessment}, 信頼度: ${confidenceLevel}%`,
			);
	...
	.andThen({
		id: "report-delivery",
		execute: async ({ data }) => {
			const analysisStatus = data.analysisCompleted
				? "completed"
				: data.marketDataStatus === "success" ||
						data.newsAnalysisStatus === "success"
					? "partial"
...
function simulateNewsCollection(cryptoId: string, requestedCount: number) {
	...
	// 人気度に基づいて記事数を調整
	const maxArticles = isPopular
		? requestedCount
		: Math.floor(requestedCount * 0.7);
	...
	return {
		status:
			actualArticles >= Math.floor(requestedCount * 0.5)
				? "success"
				: "partial",
		articlesFound: actualArticles,
		sentiment: sentiment,
		sentimentScore:
			(sentimentScores[sentiment] || 50) + (Math.random() - 0.5) * 20,
	};
}
...
function generateSummaryMessage(data: any) {
	...
	return (
		`${cryptoName}の暗号通貨分析が${status}しました。` +
